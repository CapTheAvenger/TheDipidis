...
| F-D03 | ~~MEDIUM~~ → **LOW** (closed) | Code-Konsistenz | `archetype_mapping_audit.py` | Ursprünglich 4 Files: `card_actions_builder.py` (Production-Pfad ohne Cloudflare-Bypass) wurde in PR #378 mit F-D14 entfernt; `backfill_labs_tournament_id.py` und `clean_past_meta_archetypes.py` waren One-Time-Migrationen, verifiziert komplett (110/110 `labs_tournament_id` befüllt, 0 Price-Tag-Reste) und wurden in dieser PR gelöscht. Verbleibend: `archetype_mapping_audit.py` — Operator-Audit-Tool das mit `print()` arbeitet, akzeptabel. | `awk` Output auf overview.csv + tournament_cards_data; PRs #378 + dieser | Status: **geschlossen**. |
| F-D04 | ~~MEDIUM~~ → **closed** | CI-Resilienz | `weekly-full-update.yml` | Behoben: Run-scrapers Step zählt jetzt FAIL-Markierungen + ein hartes Kriterium auf 4 kritische Scraper (`update_sets`, `all_cards_scraper`, `current_meta_analysis_scraper`, `limitless_online_scraper`). Bei > 2 Fails ODER 1 kritischem Fail wird `WEEKLY_FAIL_REASON` im env gesetzt; ein Post-Job-Step am Workflow-Ende promotet das zum Job-Failure. Einzelne sporadische Fails bleiben weiterhin `::warning::`-only — Cloudflare-Hits müssen weiterhin self-recovern. Commit + Pages-Dispatch laufen davor, damit partielle Daten trotzdem live gehen. | `weekly-full-update.yml` Run-scrapers Step + neuer `Fail job on multi-scraper / critical-scraper batch failure` Step | Status: **geschlossen**. |
...
| F-D08 | ~~MEDIUM~~ → **closed** | Daten-Lifecycle | `labs_tournament_decks__unsorted.csv` | Behoben: `scripts/sanity_check_data.py` hat eine neue `ANOMALY_WATCH` Sektion — Files die normalerweise leer sein sollten. `labs_tournament_decks__unsorted.csv` ist eingetragen mit einer 1-Zeilen-Begründung die dem Operator sagt was zu tun ist. Bei rows > 0 emit das Script jetzt eine `::warning::Data anomaly: …` Zeile die in der Job-Summary sichtbar wird. Bei 96 rows (aktueller Stand) wird die Warnung sofort sichtbar, damit der Operator triggern kann. | Phase-2 Output + neue Logik in `scripts/sanity_check_data.py` | Status: **geschlossen**. (Bei Bedarf später: GitHub-Issue-Automatisierung als zweite Stufe.) |
...
| F-D20 | ~~MEDIUM~~ → **closed** | CI-Resilienz | `weekly-full-update.yml` Step-Loop | Behoben gemeinsam mit F-D04. Der Job markiert sich jetzt als failed (= maintainer Email-Notification) bei > 2 Step-Fails oder bei jedem Fail in einem als kritisch markierten Scraper. | siehe F-D04 | Status: **geschlossen**. |
