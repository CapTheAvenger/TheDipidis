...
class CardTypeLookup:
    ...
    def load_csv_database(self) -> bool:
        ...
        possible_paths = [
            # Bevorzugt: source Ordner (wo convert_alle_karten.py es erstellt)
            os.path.join(app_path, 'all_cards_database.csv'),
            os.path.join(os.path.dirname(app_path), 'source', 'all_cards_database.csv'),
            os.path.join(app_path, '..', 'source', 'all_cards_database.csv'),
            # Legacy: Card_Database_Scraper (falls noch vorhanden)
            os.path.join(os.path.dirname(app_path), 'Card_Database_Scraper', 'all_cards_database.csv'),
        ]
    ...
    def load_card_database(self):
        ...
        if not os.path.exists(cards_file):
            print(f"WARNING: Alle Karten.txt not found at {cards_file}")
    ...
    def load_japanese_database(self):
        ...
        if not os.path.exists(japanese_file):
            print(f"WARNING: Japanische extra Karten.txt not found at {japanese_file}")
            return
