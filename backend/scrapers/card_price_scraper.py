#!/usr/bin/env python3
"""
Card Price Scraper - HYBRID STEALTH EDITION (V4 - Final Stable)
==============================================================
- Nutzt Meta-Daten & Raritäten zur CM-Minimierung.
- High-Rares & Playables: Cardmarket (SeleniumBase UC + Proxy).
- Rest (C, U, R): Präziser Set-Match via Limitless TCG.
- IPRoyal Whitelist Mode (kein Passwort-Popup).
"""
...
def _load_settings() -> dict:
    settings = load_settings("card_price_scraper_settings.json", {"delay_seconds": 5.0, "max_workers": 2, "headless": True})
    # Proxy-Settings laden
...
def _fetch_single_price(card: dict, base_delay: float, is_headless: bool, force_limitless: bool) -> dict:
    ...
    # Proxy-Settings dynamisch laden
    proxy_settings = card.get("proxy_settings")
    if not proxy_settings:
        # Fallback: hole aus globalen Settings falls nicht direkt am Card-Objekt
        proxy_settings = globals().get("_global_proxy_settings")

    proxy_str = None
    if proxy_settings:
        user = proxy_settings.get("user", "")
        pw = proxy_settings.get("pass", "")
    ...
    with _request_semaphore:
        # STRATEGIE A: Cardmarket (Nur für wichtige Karten)
        ...
        # STRATEGIE B: Limitless mit Set-Matching
        if not eur_price:
            try:
...
def scrape_prices(cards: list, settings: dict, csv_path: str) -> list:
    ...
    # Proxy-Settings global für alle Karten bereitstellen
    proxy_settings = settings.get("proxy")
    globals()["_global_proxy_settings"] = proxy_settings
    ...
    for card in cards:
        ...
        # Proxy-Settings an Card-Objekt anhängen
        card_with_proxy = dict(card)
        card_with_proxy["proxy_settings"] = proxy_settings
        to_process.append((card_with_proxy, force_lt))

    results = []
    logger.info("Starte Hybrid-Scraping: %s Karten.", len(to_process))
