#!/usr/bin/env python3
"""
City League PAST Analysis Scraper
==================================
1:1 copy of city_league_analysis_scraper.py, configured to scrape the
"rotated-out" meta from the previous JP set window. Reads its settings
from the `city_league_analysis_past` section in
`config/scraper_settings.json` (which is populated automatically by
`update_sets.apply_format_window_to_scraper_settings` the first time a
new JP set is detected — see the rotation-detection block there).

Output: `data/city_league_analysis_past.csv` (single file, overwritten
on each new rotation — the user explicitly opted for the single-file
model when the feature was specced).

Everything else (multithreaded fetching, Cloudflare bypass, archetype
canonicalisation, incremental scrape tracking) is identical to the
current-meta scraper. If a behaviour change is wanted here, mirror it
in city_league_analysis_scraper.py too unless there's a documented
...
# LOGGING SETUP
# ============================================================================
logger = setup_logging("city_league_past_scraper")
...
def get_scraped_tournaments_file() -> str:
    return os.path.join(get_data_dir(), 'city_league_analysis_past_scraped.json')
...
DEFAULT_SETTINGS = {
    "sources": {
        "city_league": {
            # Initially disabled — the past scraper has nothing to do until
            # update_sets.apply_format_window_to_scraper_settings detects
            # the first JP-set rotation post-deploy and populates this
            # block with the rotated-out meta's date window + tournament
            # IDs. When that happens it also flips `enabled` to true.
            "enabled": False,
            "start_date": "",
            "end_date": "",
    ...
    "output_file": "city_league_analysis_past.csv",
    "append_mode": True,
    "delay_between_requests": 1.5,
    "_comment": "PAST-meta variant. Scrapes the JP city-league window that ended at the day before the current set release. Populated by update_sets.apply_format_window_to_scraper_settings on rotation."
}

def _load_settings() -> dict:
    return load_settings(
        "city_league_analysis_past_settings.json", DEFAULT_SETTINGS,
...
def main():
    ...
    output_file = settings.get('output_file', 'city_league_analysis_past.csv')
