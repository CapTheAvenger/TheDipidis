#!/usr/bin/env python3
"""
City League PAST Archetype Scraper
===================================
1:1 copy of city_league_archetype_scraper.py, configured to scrape
archetype data for the rotated-out JP set window. Reads its settings
from the `city_league_archetype_past` section in
`config/scraper_settings.json` — populated automatically by
`update_sets.apply_format_window_to_scraper_settings` on the first
JP-set rotation post-deploy.

Output: `data/city_league_archetypes_past.csv` (single file, overwritten
per rotation).

Mirror behaviour changes here to city_league_archetype_scraper.py too.
"""
...
# LOGGING SETUP
# ============================================================================
logger = setup_logging("archetype_past_scraper")
...
# ============================================================================
DEFAULT_SETTINGS = {
    # PAST-meta variant — both dates are blank until the first JP-set
    # rotation populates them via update_sets.apply_format_window_to_
    # scraper_settings. Start_date will be the OLD current start_date
    # captured at rotation time; end_date will be (new jp_release - 1).
    "start_date": "",
    "end_date": "",
    "delay_between_requests": 1.5,
    "max_workers": 5,
    "output_file": "city_league_archetypes_past.csv",
    "region": "jp",
    # See current scraper's _min_size_comment in scraper_settings.json
    # for the 1-vs-32 history. JP city-league rule applies here too.
    "min_tournament_size": 1,
    "additional_tournament_ids": []
}

def _load_settings() -> dict:
    return load_settings("city_league_archetype_past_settings.json", DEFAULT_SETTINGS)
...
def merge_observed_icons_into_json() -> Dict[str, int]:
    ...
    data.setdefault("_meta", {})["lastHarvestedFrom"] = "city_league_past_archetype_scraper"
...
def main():
    ...
    # Past scraper is a no-op until the first JP-set rotation populates
    # start_date + end_date via update_sets.apply_format_window_to_
