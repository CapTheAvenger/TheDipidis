...
def _fetch_single_price(card: dict, base_delay: float, is_headless: bool, force_limitless: bool) -> dict:
    ...
    with _request_semaphore:
        ...
        # STRATEGIE B: Limitless mit PRÄZISEM Set-Matching
        if not eur_price:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if prints_table:
                        # Wir suchen exakt die Zeile, die unser Set enthält
                        for row in prints_table.select("tr"):
                            row_text = row.get_text().lower()
                            # Match auf Set-Kürzel (z.B. "asc")
                    ...
                    # Ultimer Fallback: Erster EUR Preis auf der Seite
                    if not eur_price:
...
def scrape_prices(cards: list, settings: dict, csv_path: str) -> list:
    ...
    playable_names = set()
    meta_path = os.path.join(get_data_dir(), "meta_data.json")
    if os.path.exists(meta_path):
        with open(meta_path, "r", encoding="utf-8") as f:
            meta = json.load(f)
            for deck in meta.get("decks", []):
                for c in deck.get("cards", []): playable_names.add(c.get("name", "").lower())
    
    to_process = []
    for card in cards:
        # High-Rare Check (SIR, Gold, SV, AR, etc.)
        is_high_rare = any(x in card['number'].upper() for x in ["SIR", "GG", "TG", "SV", "AR", "FA"]) or \
                       (card['number'].count('/') > 0 and int(card['number'].split('/')[0]) > 160)
        is_playable = card['name'].lower() in playable_names
