...
(async () => {
  ...
  // Switch to TEF-POR (NOT frozen): frozen share panel should NOT show
  await page.evaluate(async () => { await MetaCall._setMetaSource('past', 'TEF-POR'); });
  await page.waitForTimeout(5000);
  const tef = await page.evaluate(() => ({
    has_frozen_share: !!document.querySelector('.mc-frozen-share-panel'),
    has_field_panel: !!document.querySelector('.mc-personal-input'),
  }));
  console.log('TEF-POR (active): frozen_share_panel=', tef.has_frozen_share, 'field_panel=', tef.has_field_panel);
  ...
  console.log('Current: frozen_share_panel=', cur.has_frozen_share, 'field_panel=', cur.has_field_panel);
