...
// Part of Hausi's Pokemon TCG Analysis
        ...
        function renderCityLeagueTable(tournamentCount = 0, dateRange = '') {
            ...
            let html = `
                ...
                <div id="cityLeagueGauntletBinder"></div>
            ...
            renderCityLeagueGauntletBinder(sorted);
            ensureCityLeagueSearchFilterBinding();
            // Phase 1: render meta share chart
            renderMetaChart('cityLeague', sorted);
        }

        function getCityLeagueGauntletStorageKey() {
            return 'cityLeagueGauntletBinder';
        }

        function getCityLeagueGauntletState() {
            try {
                const raw = localStorage.getItem(getCityLeagueGauntletStorageKey());
                const parsed = raw ? JSON.parse(raw) : [];

                // Backward compatibility: older versions stored an array only.
                if (Array.isArray(parsed)) {
                    return {
                        order: parsed.map(name => String(name || '').trim()).filter(Boolean),
                        notes: {}
                    };
                }

                const order = Array.isArray(parsed?.order)
                    ? parsed.order.map(name => String(name || '').trim()).filter(Boolean)
                    : [];
                const notesRaw = parsed?.notes && typeof parsed.notes === 'object' ? parsed.notes : {};
                const notes = {};
                Object.keys(notesRaw).forEach(key => {
                    const noteKey = String(key || '').trim();
                    if (!noteKey) return;
                    notes[noteKey] = String(notesRaw[key] || '').trim();
                });

                return { order, notes };
            } catch (error) {
                console.warn('[Gauntlet] Could not parse binder data:', error);
                return { order: [], notes: {} };
            }
        }

        function saveCityLeagueGauntletBinderState(state) {
            const safeOrder = Array.from(new Set((Array.isArray(state?.order) ? state.order : [])
                .map(name => String(name || '').trim())
                .filter(Boolean)));

            const rawNotes = state?.notes && typeof state.notes === 'object' ? state.notes : {};
            const safeNotes = {};
            safeOrder.forEach(name => {
                const value = String(rawNotes[name] || '').trim();
                if (value) safeNotes[name] = value;
            });

            const safeState = { order: safeOrder, notes: safeNotes };
            localStorage.setItem(getCityLeagueGauntletStorageKey(), JSON.stringify(safeState));
            return safeState;
        }

        function getCityLeagueGauntletBinder() {
            return getCityLeagueGauntletState().order;
        }

        function saveCityLeagueGauntletBinder(list) {
            return saveCityLeagueGauntletBinderState({ order: list, notes: {} }).order;
        }
        ...
        function getCityLeagueGauntletRankValue(deck) {
            const rankRaw = deck.new_avg_placement || deck.avg_placement || deck.average_placement || 0;
            const parsed = parseFloat(String(rankRaw).replace(',', '.'));
            return Number.isFinite(parsed) ? parsed : 0;
        }

        function renderCityLeagueGauntletBinder(sortedData) {
            const container = document.getElementById('cityLeagueGauntletBinder');
            if (!container) return;

            const sorted = Array.isArray(sortedData) ? sortedData : [];
            const candidates = sorted.slice(0, 15);
            const candidateByName = new Map(candidates.map(deck => [String(deck.archetype || '').trim(), deck]));
            const allByName = new Map(sorted.map(deck => [String(deck.archetype || '').trim(), deck]));

            const state = getCityLeagueGauntletState();
            const validStored = state.order.filter(name => allByName.has(name));
            const validNotes = {};
            validStored.forEach(name => {
                if (state.notes[name]) validNotes[name] = state.notes[name];
            });
            if (validStored.length !== state.order.length || Object.keys(validNotes).length !== Object.keys(state.notes || {}).length) {
                saveCityLeagueGauntletBinderState({ order: validStored, notes: validNotes });
            }

            let html = `
                <section class="meta-gauntlet-binder" aria-label="${escapeHtml(t('cl.gauntletAria'))}">
                    <div class="meta-gauntlet-header">
                        <div>
                            <h3>${t('cl.gauntletTitle')}</h3>
                            <p>${t('cl.gauntletSubtitle')}</p>
                        </div>
                        <div class="meta-gauntlet-header-actions">
                            <button class="btn-modern" onclick="autoFillCityLeagueGauntletBinder()">${t('cl.gauntletAutoFill')}</button>
                            <button class="btn-modern tertiary-danger" onclick="clearCityLeagueGauntletBinder()">${t('cl.gauntletClear')}</button>
                        </div>
                    </div>
                    <div class="meta-gauntlet-content">
                        <div class="meta-gauntlet-candidates">
                            <h4>${t('cl.gauntletCandidates')}</h4>
                            <div class="meta-gauntlet-cards">`;

            candidates.forEach(deck => {
                const name = String(deck.archetype || '').trim();
                if (!name) return;
                const count = parseInt(deck.new_count || 0, 10) || 0;
                const avg = getCityLeagueGauntletRankValue(deck).toFixed(1);
                const inBinder = validStored.includes(name);
                const escapedName = escapeJsStr(name);
                html += `
                    <article class="meta-gauntlet-card ${inBinder ? 'is-selected' : ''}">
                        <div class="meta-gauntlet-card-name">${name}</div>
                        <div class="meta-gauntlet-card-stats">
                            <span class="stat-badge">📦 ${count}</span>
                            <span class="stat-badge">🏆 ${avg}</span>
                        </div>
                        <div class="meta-gauntlet-card-actions">
                            <button class="btn-modern ${inBinder ? 'success' : ''}" onclick="toggleCityLeagueGauntletDeck('${escapedName}')">
                                ${inBinder ? t('cl.gauntletPinned') : t('cl.gauntletPin')}
                            </button>
                            <button class="btn-modern" onclick="jumpToCardAnalysis('${escapedName}', 'cityLeague')">${t('cl.gauntletAnalyze')}</button>
                        </div>
                    </article>`;
            });
            ...
            html += `
                        ...
                        <div class="meta-gauntlet-selected">
                            <h4>${t('cl.gauntletSelected')} (${validStored.length})</h4>`;

            if (validStored.length === 0) {
                html += `
                            <div class="meta-gauntlet-empty">${t('cl.gauntletEmpty')}</div>`;
            } else {
                html += '<ul class="meta-gauntlet-list">';
                validStored.forEach((name, idx) => {
                    const escapedName = escapeJsStr(name);
                    const note = validNotes[name] || '';
                    const noteEscaped = escapeHtmlText(note);
                    const deck = allByName.get(name) || {};
                    const count = parseInt(deck.new_count || 0, 10) || 0;
                    const avg = getCityLeagueGauntletRankValue(deck).toFixed(1);
                    html += `
                        <li class="meta-gauntlet-list-item" draggable="true" ondragstart="startCityLeagueGauntletDrag(event, '${escapedName}')" ondragend="endCityLeagueGauntletDrag(event)" ondragover="allowCityLeagueGauntletDrop(event)" ondrop="dropCityLeagueGauntletDrag(event, '${escapedName}')">
                            <span class="meta-gauntlet-list-rank">${idx + 1}</span>
                            <div class="meta-gauntlet-list-main">
                                <div class="meta-gauntlet-list-title-row">
                                    <span class="meta-gauntlet-list-name">${name}</span>
                                    <span class="meta-gauntlet-list-meta">📦 ${count} · 🏆 ${avg}</span>
                                </div>
                                <textarea class="meta-gauntlet-note-input" placeholder="${escapeHtml(t('cl.gauntletNotePlaceholder'))}" oninput="updateCityLeagueGauntletNote('${escapedName}', this.value)">${noteEscaped}</textarea>
                            </div>
                            <div class="meta-gauntlet-list-actions" ondragover="allowCityLeagueGauntletDrop(event)">
                                <button class="btn-modern" onclick="loadCityLeagueGauntletDeck('${escapedName}')">${t('cl.gauntletLoad')}</button>
                                <button class="btn-modern" onclick="jumpToCardAnalysis('${escapedName}', 'cityLeague')">${t('cl.gauntletAnalyze')}</button>
                                <button class="btn-modern tertiary-danger" onclick="toggleCityLeagueGauntletDeck('${escapedName}')">${t('cl.gauntletRemove')}</button>
                            </div>
                        </li>`;
                });
                html += '</ul>';
            ...
            html += `
                            <div class="meta-gauntlet-footer-actions">
                                <button class="btn-modern primary" onclick="openCityLeagueGauntletCompare()" ${validStored.length < 2 ? 'disabled' : ''}>${t('cl.gauntletOpenCompare')}</button>
                                <button class="btn-modern" onclick="copyCityLeagueGauntletBinder()" ${validStored.length === 0 ? 'disabled' : ''}>${t('cl.gauntletCopy')}</button>
                            </div>
                        </div>
                    </div>
                </section>`;
        ...
        function toggleCityLeagueGauntletDeck(archetypeName) {
            const name = String(archetypeName || '').trim();
            if (!name) return;
            const state = getCityLeagueGauntletState();
            const binder = state.order;
            const exists = binder.includes(name);
            const next = exists ? binder.filter(item => item !== name) : [...binder, name];

            const nextNotes = { ...(state.notes || {}) };
            if (exists) delete nextNotes[name];

            saveCityLeagueGauntletBinderState({ order: next, notes: nextNotes });
            renderCityLeagueGauntletBinder(window.cityLeagueSortedData || []);
        }

        function clearCityLeagueGauntletBinder() {
            saveCityLeagueGauntletBinderState({ order: [], notes: {} });
            renderCityLeagueGauntletBinder(window.cityLeagueSortedData || []);
        }

        function autoFillCityLeagueGauntletBinder() {
            const sorted = Array.isArray(window.cityLeagueSortedData) ? window.cityLeagueSortedData : [];
            const top = sorted.slice(0, 6).map(deck => String(deck.archetype || '').trim()).filter(Boolean);
            const existing = getCityLeagueGauntletState();
            const mergedNotes = {};
            top.forEach(name => {
                if (existing.notes[name]) mergedNotes[name] = existing.notes[name];
            });
            saveCityLeagueGauntletBinderState({ order: top, notes: mergedNotes });
            renderCityLeagueGauntletBinder(sorted);
        }

        function updateCityLeagueGauntletNote(archetypeName, noteValue) {
            const name = String(archetypeName || '').trim();
            if (!name) return;
            ...
            if (cleanNote) {
                nextNotes[name] = cleanNote;
            } else {
                delete nextNotes[name];
            }
            saveCityLeagueGauntletBinderState({ order: state.order, notes: nextNotes });
        }

        function allowCityLeagueGauntletDrop(event) {
            event.preventDefault();
        }

        function startCityLeagueGauntletDrag(event, archetypeName) {
            const name = String(archetypeName || '').trim();
            if (!name) return;
            window.cityLeagueGauntletDraggedName = name;
        ...
        function endCityLeagueGauntletDrag(event) {
            if (event?.currentTarget) {
                event.currentTarget.classList.remove('is-dragging');
            }
            window.cityLeagueGauntletDraggedName = null;
        }

        function dropCityLeagueGauntletDrag(event, targetArchetypeName) {
            event.preventDefault();
            const target = String(targetArchetypeName || '').trim();
            if (!target) return;

            const source = window.cityLeagueGauntletDraggedName || (event?.dataTransfer ? event.dataTransfer.getData('text/plain') : '');
            const sourceName = String(source || '').trim();
            if (!sourceName || sourceName === target) return;

            const state = getCityLeagueGauntletState();
            const order = [...state.order];
            const sourceIdx = order.indexOf(sourceName);
            const targetIdx = order.indexOf(target);
            if (sourceIdx < 0 || targetIdx < 0) return;

            order.splice(sourceIdx, 1);
            order.splice(targetIdx, 0, sourceName);
            saveCityLeagueGauntletBinderState({ order, notes: state.notes });
            renderCityLeagueGauntletBinder(window.cityLeagueSortedData || []);
        }

        function loadCityLeagueGauntletDeck(archetypeName) {
            const name = String(archetypeName || '').trim();
            if (!name) return;
            const select = document.getElementById('cityLeagueDeckSelect');
            if (select) select.value = name;
            if (typeof loadCityLeagueDeckData === 'function') {
                loadCityLeagueDeckData(name);
            }
        }

        function openCityLeagueGauntletCompare() {
            if (typeof openDeckCompare === 'function') {
                openDeckCompare('cityLeague');
            }
        }

        async function copyCityLeagueGauntletBinder() {
            const state = getCityLeagueGauntletState();
            const binder = state.order;
            if (binder.length === 0) return;
            const text = binder.map((name, index) => {
                const note = String(state.notes[name] || '').trim();
                return note
                    ? `${index + 1}. ${name} - Note: ${note}`
                    : `${index + 1}. ${name}`;
            }).join('\n');
            try {
                await navigator.clipboard.writeText(text);
                if (typeof showToast === 'function') showToast(t('cl.gauntletCopied'));
            } catch (error) {
                console.warn('[Gauntlet] Clipboard copy failed:', error);
            }
        }
        ...
        window.toggleCityLeagueGauntletDeck = toggleCityLeagueGauntletDeck;
        window.clearCityLeagueGauntletBinder = clearCityLeagueGauntletBinder;
        window.autoFillCityLeagueGauntletBinder = autoFillCityLeagueGauntletBinder;
        window.loadCityLeagueGauntletDeck = loadCityLeagueGauntletDeck;
        window.openCityLeagueGauntletCompare = openCityLeagueGauntletCompare;
        window.copyCityLeagueGauntletBinder = copyCityLeagueGauntletBinder;
        window.updateCityLeagueGauntletNote = updateCityLeagueGauntletNote;
        window.allowCityLeagueGauntletDrop = allowCityLeagueGauntletDrop;
        window.startCityLeagueGauntletDrag = startCityLeagueGauntletDrag;
        window.endCityLeagueGauntletDrag = endCityLeagueGauntletDrag;
        window.dropCityLeagueGauntletDrag = dropCityLeagueGauntletDrag;
