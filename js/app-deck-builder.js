...
try { localStorage.removeItem('autosave_deck'); } catch (_) {}
        ...
        async function autoCompleteConsistency(source, rarityMode) {
            ...
            // Meta-share-tiered tech budget per active threat category.
            // Per user feedback on the previous "always cap at 1": rigid
            // ceiling is wrong — when the meta runs the threat in >50% of
            // matches a single out is too thin (you'll see retreat lock
            // multiple times in a 7-round bracket), but at <30% one is
            // plenty. Tier the target on weighted_meta_share:
            //
            //   < 30 %  →  1 out  (face threat in ~2 of 7 rounds)
            //   30–60 % →  2 outs (~3-4 rounds)
            //   ≥ 60 %  →  3 outs
            //
            // Then greedy-fill counter cards (sorted by archetype share
            // desc) until the budget is met. Each picked card carries a
            // _techCounterMaxCount cap that the Stage-1 / Stage-2 push
            // respects, so two counters whose natural addCounts would
            // sum past the budget get trimmed to fit.
            //
            // The chosen counter per category is the one with the
            // highest archetype share, with non-ace-spec preferred on
            // a tie so the ace-spec slot still goes to the deck's
            // primary ace-spec choice.
            ...
            if (techAuditActiveThreats.size > 0 && techAuditCounterCats.size > 0) {
                ...
                techAuditActiveThreats.forEach((info, cat) => {
                    ...
                    candidates.sort((a, b) => {
                        ...
                        // Tie-break: non-ace-spec wins so the ace-spec slot
                        // stays free for the deck's primary ace-spec choice.
                    ...
                    for (const c of candidates) {
                        ...
                        // Track the TIGHTEST budget across categories — a
                        // card that counters two categories (e.g. Iono
                        // covering hand_disruption only) gets the larger
                        // allowance from whichever category needs more.
