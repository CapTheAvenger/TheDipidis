...
window.MetaCall = (function () {
  ...
  function _runPredictor() {
    ...
    // famConvProjection (the conversion-weighted target). Push or pull
    // proportionally on every non-family deck so the renorm still sums
    // to 100. Skips families whose current total is already at/above
    // the blend target (cap path handles those).
    Object.keys(familyAgg).forEach(fam => {
      const f = familyAgg[fam];
      ...
      const othersTotal  = 100 - currentTotal;
      if (othersTotal <= 0) return;
      const liftPP       = target - currentTotal;
      const memberScale  = target / currentTotal;
      const othersScale  = (othersTotal - liftPP) / othersTotal;
      if (othersScale <= 0) return; // pathological: skip rather than negative
      const famSet = new Set(f.members);
      f.members.forEach(d => {
      ...
      try {
        console.log(
          ...
          `family ×${memberScale.toFixed(3)}, others ×${othersScale.toFixed(3)}.`
        );
      } catch (_e) { /* ignore */ }
      // Diagnostic on every family member.
      f.members.forEach(d => {
        if (d._tier1Diag) {
