...
window.MetaCall = (function () {
  ...
  const TIER1_CONVERGENCE_THRESHOLD = 10.0;  // rawLadderPct ≥ this counts as Tier 1
  const TIER1_CONSOLIDATION_RATIO   = 1.3;   // lastMajorShare ≥ median × this confirms consolidation
  ...
  const TIER1_BLEND_WEIGHT          = 0.5;   // 50/50 blend with existing ladderPct
  const TIER1_REGIONAL_SPIKE_GUARD  = 0.5;   // online < lastMajorShare × this → skip boost
  ...
  function _runPredictor() {
    ...
    _shareList.forEach(d => {
      ...
      // Predictor 6.0 — Tier-1 Convergence Detector (see constants
      // section above for derivation + post-NAIC backtest evidence).
      // Tracks the diagnostic shape used by the per-deck inspector so
      // a reviewer can see why a Dragapult-shaped prediction jumped.
      d._tier1ConvergenceBoost = null;
      const lmStats = _lastMajorByDeck[k];
      const lmShare = lmStats ? lmStats.share : 0;
      const lmConv  = lmStats ? lmStats.dayConv : 0;
      const lmD1    = lmStats ? lmStats.day1Players : 0;
      if (rawLadderPct >= TIER1_CONVERGENCE_THRESHOLD
        ...
        const convMult       = 1 + TIER1_CONV_DAMPING * (lmConv / _meanDay2Conv - 1);
        const convProjection = lmShare * convMult;
        const boosted        = ladderPct * (1 - TIER1_BLEND_WEIGHT)
                             + convProjection * TIER1_BLEND_WEIGHT;
        d._tier1ConvergenceBoost = {
          lastMajorShare: lmShare,
          majorMedian:    majorMedian,
          convMult:       convMult,
          convProjection: convProjection,
          fromLadderPct:  ladderPct,
          toLadderPct:    boosted,
        };
        ladderPct = boosted;
      }
