// @ts-check
/*
 * archetype-icons.js — resolves meta archetype names to Limitless R2 icon URLs.
 *
 * Wave-1 L2.11 — converted from legacy IIFE (js/archetype-icons.js) to a
 * proper ES module. The Public API now exports a single `ArchetypeIcons`
 * namespace object; the modules-bundle footer re-attaches it as
 * `window.ArchetypeIcons` so legacy non-module callers keep working
 * unchanged.
 *
 * Usage:
...
const CACHE_TOKEN = (typeof document !== 'undefined' && document.currentScript)
  ? ((/** @type {HTMLScriptElement} */ (document.currentScript)).src.match(/[?&]v=([^&]+)/) || [null, 'dev'])[1]
  : 'dev';
