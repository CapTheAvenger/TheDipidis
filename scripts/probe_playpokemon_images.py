...
def probe_series(s, loc, lang, max_n=90, stop_after_misses=8):
    found, misses = [], 0
    sample_ct = None
    for n in range(1, max_n + 1):
        ...
        if st in (200, 206) and ispng:
            ...
            misses += 1
            if misses >= stop_after_misses and (found or n > 12):
                break
    return found, sample_ct


def main():
    print("== play.pokemon.com CDN probe (no Referer sent) ==")
    total = 0
    for s in range(1, 10):
        for loc, lang in (("de-de", "DE"), ("en-us", "EN")):
            found, ct = probe_series(s, loc, lang)
            total += len(found)
            if found:
                ...
                print(f"SE{s:<2} {loc}: {len(found):>3} imgs (num {lo}-{hi}, "
                      f"contiguous={contiguous}, ct={ct!r})  e.g. {one}")
            else:
                print(f"SE{s:<2} {loc}: none")
    print(f"TOTAL images that loaded (PNG, no Referer): {total}")
    return 0
